...
from src.gemini_client import GeminiClient, fallback_extract_slots, fallback_followup
from src.note_builder import build_note, build_uncertain_note
from src.rule_engine import RuleEngine, red_flag_matches
from src.schemas import ExtractedSlots, Message, Session, SessionResponse, TriageNote
from src.slot_validation import has_confident_category, has_target_slot_value, validate_slots
...
class SessionManager:
    ...
    def _handle_turn(self, session: Session, patient_text: str) -> SessionResponse:
        source = (
            "reported"
            if not any(message.role == "patient" for message in session.transcript)
            else "followup"
        )
        ...
        extracted = self._extract_without_generation(
            session,
            patient_text,
            target_slot,
        )
        ...
        if (
            session.slots.complaint_category == "unclear"
            or session.slots.confidence < 0.5
        ):
        ...
        if session.slots.complaint_category == "unclear":
            ...
            if session.unclear_count >= 2:
                return self._escalate_uncertain(
                    session,
                    ["complaint_category"],
                )
        ...
        # ------------------------------------------------------------------
        # Safety-critical backend red-flag scan.
        #
        # Do not rely only on Gemini putting a red flag into the correct
        # associated_symptoms slot. Scan the patient's actual latest answer
        # against the deterministic red flags configured for this category.
        # ------------------------------------------------------------------
        detected_red_flags = self._detect_red_flags_in_text(
        ...
        if detected_red_flags:
            for flag in detected_red_flags:
                if flag not in session.slots.associated_symptoms:
                    session.slots.associated_symptoms.append(flag)

            session.slots.associated_symptoms = sorted(
                set(session.slots.associated_symptoms)
            )

            session.slot_sources["associated_symptoms"] = source

        # Immediate escalation MUST happen before missing-slot questions.
        ...
        if target_slot and session.followup_count < MAX_FOLLOWUPS:
            ...
            if target_slot == "associated_symptoms":
                red_flag_examples = self.rule_engine.red_flags_for(
                    session.slots.complaint_category
                )

            return self._ask(
                session,
                fallback_followup(target_slot, red_flag_examples),
    ...
    def _extract_without_generation(
        self,
        session: Session,
        patient_text: str,
        target_slot: str | None,
    ) -> ExtractedSlots:
        deterministic = validate_slots(
            fallback_extract_slots(patient_text, target_slot=target_slot)
        )

        if target_slot:
            if has_target_slot_value(deterministic, target_slot):
                return deterministic
        elif has_confident_category(deterministic):
            return deterministic

        if not target_slot and deterministic.complaint_category == "unclear":
            guessed = self.embedding_index.classify(patient_text)
            if guessed:
                deterministic.complaint_category = guessed
                deterministic.confidence = max(deterministic.confidence, 0.65)
                return validate_slots(deterministic)

        if target_slot and has_target_slot_value(deterministic, target_slot):
            return deterministic
        if not target_slot and deterministic.complaint_category != "unclear":
            return deterministic
    ...
    def _detect_red_flags_in_text(
        self,
        category: str,
        patient_text: str,
    ) -> list[str]:
        """
        Deterministically scan the patient's latest answer for red flags
        configured for the detected complaint category.

        This is a safety net around LLM extraction. It does not infer
        diagnoses or create new clinical rules.
        """
        detected: list[str] = []

        for flag in self.rule_engine.red_flags_for(category):
            if red_flag_matches(flag, patient_text):
    ...
    def _escalate_uncertain(
        ...
        session.note = build_uncertain_note(
            session.slots,
            unknowns,
            session.slot_sources,
            self.gemini,
    ...
    def _merge_slots(
        ...
        for key, value in update.model_dump().items():
            ...
            if key == "associated_symptoms":
                if value:
                    current.associated_symptoms = sorted(
                        set(current.associated_symptoms + value)
                    )
    ...
    def _get_session(self, session_id: str) -> Session:
        ...
        note = (
            TriageNote.model_validate(record["note"])
            if record["note"]
            else None
        )
        ...
        session = Session(
            ...
            pending_slot=(
                session_row["pending_slot"]
                if "pending_slot" in session_row.keys()
                else None
            ),
            asked_slots=(
                json.loads(session_row["asked_slots_json"])
                if "asked_slots_json" in session_row.keys()
                else []
            ),
            transcript=[
            ...
            slots=ExtractedSlots.model_validate_json(
                session_row["slots_json"]
            ),
            slot_sources=dict(
                json.loads(session_row["slot_sources_json"])
            ),
            note=note,
        )
